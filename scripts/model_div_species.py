...
data = pd.read_csv('../data/dataset2_het_froh_species.csv')
...
plt.show()
fig.savefig("../output/Quality_Control_Features_Multi_Species", dpi=500)
...
data[['species_encoded_1', 'species_encoded_2', 'species_encoded_3', 'species_encoded_4', 'species_encoded_5', 'species_encoded_6']]=species_encoded # number of categories determined the number of new features added, here 6
...
if os.path.exists('multi_species_modeling/best_model.keras'):
    
    print('The model has already been trained and saved on disk!')
    
    best_model=tf.keras.models.load_model('multi_species_modeling/best_model.keras')
    
elif os.path.exists('multi_species_modeling/best_checkpoint.keras'):
    
    print('The model has already been trained and saved on disk!')
    
    best_model=tf.keras.models.load_model('multi_species_modeling/best_checkpoint.keras')
...
print('The predictions are: ', species_pred)est_model.fit(X_train, y_train, validation_data=(X_valid, y_valid), epochs=10)
